...
import com.teacup.teacuppicturebackend.manager.websocket.disruptor.PictureEditEventProducer;
import com.teacup.teacuppicturebackend.manager.websocket.model.PictureEditActionEnum;
...
public class PictureEditHandler extends TextWebSocketHandler {
    ...
    @Lazy
    /**
     * PictureEditHandler 实例化完成后，再注入 pictureEditEventProducer
     * pictureEditEventProducer 中的 Disruptor 已经配置好 PictureEditEventWorkHandler
     * 整个链条在Spring容器启动阶段就能完成初始化
     */
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        //将对应的用户会话和图片id存入pictureSessions(Map)
    ...
    private void broadcastToPicture(Long pictureId, PictureEditResponseMessage pictureEditResponseMessage, WebSocketSession excludeSession) throws IOException {
        ...
        if (CollUtil.isNotEmpty(sessionSet)) {
            ...
            //前端 JS 的长整数可能会丢失精度，所以使用 Jackson 自定义序列化器，在将对象转换为 JSON 字符串时，将 Long 类型转换为 String 类型。
